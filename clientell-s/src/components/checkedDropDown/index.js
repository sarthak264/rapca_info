...
export const CheckedDropDown = ({ data, name, checkboxFunction }) => {
    const { title, items, defaultSelected } = data;
    const [currentOption, setCurrentOption] = useState(defaultSelected);
    const dispatch = useDispatch()
    const usersProcessList = useSelector(state => state.userdropdownmanager.value)
    // useEffect(() => {
    ...
    return (
        ...
        >
            <div className="checknames">
                <input type="checkbox" name="" id={'usercheck' + item.id}
                    defaultChecked={usersProcessList.includes(item.id) ? true : false}
                    onChange={(e) => {
            ...
            <div className="subdropdowndivEle">
                {mouseEnterState ? <SubDropDown id={item.id} subOrdinateState={item.has_subordinates} /> : null}
            </div>
        </div>
    );
};
...
const SubDropDown = ({ id, name, subOrdinateState }) => {
    ...
    return subOrdinateState ? checkUserList.length === 0 ? (
        <Reactloading
            width={15}
            height={15}
            color="#101012"
...
const SubDropDownLevel = ({ item, index, id }) => {
    ...
    const dispatch = useDispatch()
    return (
        <div className="subdropdownCustomCheckboxDiv" key={index}
            onMouseEnter={(e) => {
                e.preventDefault();
        ...
        >
            ...
            {subCheckboxState && id !== null ? (
                <div className="subdropdowndiv1">
                    {" "}
                    <SubDropDown id={id} subOrdinateState={item.has_subordinates} />
                </div>
            ) : null}
        </div>
    );
